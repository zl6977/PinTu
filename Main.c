...
int step(int d1, int d2, int d3, int d4, int i, int j, int od)
{
  ...
  if (DISPLAY == 1)
    printfmap();
add5:
  if (d1 == 1 && move[i + 1][j] != 1 && od != 2)
  {
    up(i - 1, j - 1);
    return 1;
  }
  if (d1 == 2 && move[i - 1][j] != 1 && od != 1)
  {
    down(i - 1, j - 1);
    return 2;
  }
  if (d1 == 3 && move[i][j - 1] != 1 && od != 4)
  {
    left(i - 1, j - 1);
    return 3;
  }
  if (d1 == 4 && move[i][j + 1] != 1 && od != 3)
  {
    right(i - 1, j - 1);
    return 4;
  }
  if (d2 == 1 && move[i + 1][j] != 1 && od != 2)
  {
    up(i - 1, j - 1);
    return 1;
  }
  if (d2 == 2 && move[i - 1][j] != 1 && od != 1)
  {
    down(i - 1, j - 1);
    return 2;
  }
  if (d2 == 3 && move[i][j - 1] != 1 && od != 4)
  {
    left(i - 1, j - 1);
    return 3;
  }
  if (d2 == 4 && move[i][j + 1] != 1 && od != 3)
  {
    right(i - 1, j - 1);
    return 4;
  }
  if (d3 == 1 && move[i + 1][j] != 1 && od != 2)
  {
    up(i - 1, j - 1);
    return 1;
  }
  if (d3 == 2 && move[i - 1][j] != 1 && od != 1)
  {
    down(i - 1, j - 1);
    return 2;
  }
  if (d3 == 3 && move[i][j - 1] != 1 && od != 4)
  {
    left(i - 1, j - 1);
    return 3;
  }
  if (d3 == 4 && move[i][j + 1] != 1 && od != 3)
  {
    right(i - 1, j - 1);
    return 4;
  }
  if (d4 == 1 && move[i + 1][j] != 1 && od != 2)
  {
    up(i - 1, j - 1);
    return 1;
  }
  if (d4 == 2 && move[i - 1][j] != 1 && od != 1)
  {
    down(i - 1, j - 1);
    return 2;
  }
  if (d4 == 3 && move[i][j - 1] != 1 && od != 4)
  {
    left(i - 1, j - 1);
    return 3;
  }
  if (d4 == 4 && move[i][j + 1] != 1 && od != 3)
  {
    right(i - 1, j - 1);
    return 4;
  }
...
add3:
  ...
  while (i != i1 || j != j1)
  {
    ...
    if (i < i1 && j < j1)
      d = step(4, 1, 3, 2, i, j, d);
    if (i > i1 && j < j1)
      d = step(2, 4, 1, 3, i, j, d);
    if (i < i1 && j > j1)
      d = step(3, 1, 2, 4, i, j, d);
    if (i > i1 && j > j1)
      d = step(3, 2, 4, 1, i, j, d);
    if (i == i1 && j > j1)
      d = step(3, 1, 2, 4, i, j, d);
    if (i == i1 && j < j1)
      d = step(4, 2, 1, 3, i, j, d);
    if (i > i1 && j == j1)
      d = step(2, 4, 3, 1, i, j, d);
    if (i < i1 && j == j1)
      d = step(1, 4, 3, 2, i, j, d);
...
struct graph *up(int i, int j, struct graph *p)
{
  ...
  q->step = p->step + 1;
  if (q->g[0][0] == 0 && q->g[0][1] == 1 && q->g[0][2] == 2 && q->g[1][0] == N)
  {
    success(q->zong, q->step, 1);
    return NULL;
  }
  q->pre = 1; //上一步记为向上
  for (i0 = 1; i0 < q->step; i0++)
    q->zong[i0] = p->zong[i0];
...
struct graph *right(int i, int j, struct graph *p)
{
  ...
  q->step = p->step + 1; //步数加1
  if (q->g[0][0] == 0 && q->g[0][1] == 1 && q->g[0][2] == 2 && q->g[1][0] == N && q->g[1][1] == N + 1)
  {
    success(p->zong, q->step, 4);
    return NULL;
  }
  q->pre = 4; //上一步的方向记为向右
  for (i0 = 1; i0 < q->step; i0++)
...
void modify()
{
  ...
  for (i = 0; i < N; i++)
  {
    ...
    {
      if (map[i][j] == 0)
        tmap[N - 1 - i][N - 1 - j] = map[i][j];
      if (map[i][j] != 0)
        tmap[N - 1 - i][N - 1 - j] = 16 - map[i][j];
    }
  }
  for (i = 0; i < N; i++)
  {
